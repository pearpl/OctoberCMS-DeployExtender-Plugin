...
{
    protected $signature = 'deployextender:sync-pull
    ...
    protected $description = 'Pull remote database and storage files to the local server';
    ...
    public function handle()
    {
        ...
        if ($lastSync) {
            $this->line('');
            $this->info("Last pull sync: {$lastSync->completed_at->format('Y-m-d H:i:s')}");
        }

        $this->line('');
        $this->warn('⚠ WARNING: This will OVERWRITE local data with the remote server data!');
        $this->warn('  A backup of the local database will be created before sync.');
        ...
        try {
            if ($syncDb) {
                ...
                $dbResult = $manager->pullDatabase($skipUsers);
            ...
            if ($syncMedia) {
                ...
                $totalFiles += $manager->pullStorage('app/media');
            }

            if ($syncUploads) {
                $this->line('');
                $this->info('--- Uploads Sync ---');
                $totalFiles += $manager->pullStorage('app/uploads');
            }

            $syncLog->markSuccess($totalTables, $totalFiles, $backupPath);

            $this->line('');
            $this->info('✓ Sync pull completed successfully!');
            $this->line("  Tables synced: {$totalTables}");
            $this->line("  Files synced: {$totalFiles}");
            if ($backupPath) {
                $this->line("  Local backup: {$backupPath}");
            }
